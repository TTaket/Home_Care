...
def OutFile(Path , Name):
    ...
    #检查缓冲区答案文件是否存在
    flag1 = os.path.exists(conf.SAT_ANSFILE)
    if flag1 == False:
    ...
    # 获取文件所在目录和完整文件名
    _, full_file_name = os.path.split(conf.SAT_ANSFILE)
    file_name, file_ext = os.path.splitext(conf.SAT_ANSFILE)
    
    #可以正常读写
    source = conf.SAT_ANSFILE
    target = Path +'/' +Name
